...
async function api(path, options={}) {
  ...
  if(key) options.headers = {...(options.headers || {}), 'X-API-Key': key};
...
async function pollJob(id){
  try {
    ...
    if(data.status === 'queued' || data.status === 'running'){
      ...
      if(name) $('predictExperiment').value = name;
      if(name) $('generateExperiment').value = name;
    }
  } catch(e) {
    $('jobResult').textContent = e.message;
  }
}
